...
template<typename Type>
Type &List<Type>::Node::getData() const {
    return data;
}
...
template<typename Type>
List<Type>::List(const List &l) : anchor(nullptr) {
    copyAll(l);
}
...
template<typename Type>
void List<Type>::insert(List<Type>::Node *p, const Type &e) {//modify
...
template<typename Type>
void List<Type>::erase(List<Type>::Node *p) {//modify
    if(!validPos(p)) {
