...
//    int count = 0; ///con esto vere el inicio y fin de los parentesis
    ...
    while (!cola.empty()) {
        ...
        if(operatorValid(cola.getFront())) {
            ...
            if(precedencia(cola.getFront()) < 4) {
                /*si es un operador*/
                while(!pila.isEmpty() and precedencia(pila.getTop()) >= precedencia(cola.getFront()) and precedencia(pila.getTop() != 4)) {
                    /*mientras que la pila no este vacia y su tope tenga una precedencia mayor*/
