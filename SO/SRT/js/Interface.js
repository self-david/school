class Interface {
    ...
    static deleta(target) {
        if (target.classList.contains('delete')) {   
            let data = target.parentElement.parentElement;
            window.localStorage.removeItem(data.id);
            data.remove();
        } else if(target.classList.contains('delete-icon')) {
            let data = target.parentElement.parentElement.parentElement;
            window.localStorage.removeItem(data.id);
            data.remove();
        }
    }
    ...
    static showProcessProgressBar(scheduler) {
        ...
        for (let i = 0; i < scheduler.processos.length; i++) {
            ...
            div.innerHTML = `
                ...
                <label class="float-right" id="status-${processo.id}">Status: ${processo.status}. Tiempo Restante: ${
    ...
    static async updateProcessStatus(scheduler) {
        for (let i = 0; i < scheduler.processos.length; i++) {
            ...
            statusLabel.innerHTML = `Status: ${process.status}. Tiempo Restante: ${process.tempoRestante}`;
        }
    }
